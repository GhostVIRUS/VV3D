...
class voronoicell_base {
	public:
		...
		/*
		// combining output_face_vertices and voro_print_face_vertices
		inline void output_face_polygons(double x, double y, double z, std::vector<std::vector<std::vector<float>>> &cell) {
			std::vector<int> v; face_vertices(v);
			int j, k = 0, l;
			...
			if (v.size() > 0) {
				...
				if (l <= 1) {
					if (l == 1)
					{
						...
						point.push_back(x + 0.5*pts[3 * v[k]]);
						point.push_back(y + 0.5*pts[3 * v[k] + 1]);
						point.push_back(z + 0.5*pts[3 * v[k] + 2]);

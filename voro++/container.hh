...
class container : public container_base, public radius_mono {
	public:
		...
		/*
		template<class c_loop>
		void output_cells(c_loop &vl, std::vector<std::vector<std::vector<std::vector<float>>>> &cellgroup) {
			voronoicell c; double *pp;
			if (vl.start()) do if (compute_cell(c, vl)) {
				std::vector<std::vector<std::vector<float>>> cd;
				pp = p[vl.ijk] + ps*vl.q;
				c.output_face_polygons(*pp, pp[1], pp[2], cd);
				cellgroup.push_back(cd);
			} while (vl.inc());
		}

		void output_cells(std::vector<std::vector<std::vector<std::vector<float>>>> &cellgroup){
			c_loop_all vl(*this);
			output_cells(vl, cellgroup);
		}*/
	private:

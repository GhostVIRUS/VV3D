...
void voronoicell_base::add_memory(vc_class &vc, int i, int *stackp2) {
	...
	if (mem[i] == 0) {
		...
		int j = 0, k, *l;
		mem[i] <<= 1;
		//if(mem[i]>max_n_vertices) //memory error
		l = new int[s*mem[i]];
		int m = 0;
...
void voronoicell_base::add_memory_vertices(vc_class &vc) {
	...
	//if (i > max_vertices) //memory error
	double *ppts;
	pp = new int*[i];
...
void voronoicell_base::add_memory_vorder(vc_class &vc) {
	...
	//if(i>max_vertex_order) //memory error
	p1 = new int[i];
...
void voronoicell_base::add_memory_ds(int *&stackp) {
	...
	//if(current_delete_size>max_delete_size) //memory error
...
void voronoicell_base::add_memory_ds2(int *&stackp2) {
	...
	//if(current_delete2_size>max_delete2_size) //memory error
...
bool voronoicell_base::nplane(vc_class &vc, double x, double y, double z, double rsq, int p_id) {
	...
	// Test approximately sqrt(n)/4 points for their proximity to the plane and keep the one which is closest
	uw = m_test(up, u);
	...
	try {
		if (uw == 1) {
			...
			// If the last point in the iteration is within the plane, we need to do the complicated setup routine. Otherwise, we use the regular iteration.
			if (lw == 0) {
				up = lp;
			...
			// Our original test point was on the plane, so we automatically head for the complicated setup routine
			complicated_setup = true;
		}
	}
	catch (bool except) {
